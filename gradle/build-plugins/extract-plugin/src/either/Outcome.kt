...
inline fun <F, F2, S, S2> Outcome<F, S>.flatMap(f: (S) -> Either<F2, S2>): Outcome<F2, S2> where F : F2 = flatMapRight(f)
inline fun <F, F2, S, S2> Outcome<F, S>.flatMapFailure(f: (F) -> Either<F2, S2>): Outcome<F2, S2> where S : S2 = flatMapLeft(f)
